...
type SessionState = {
  ...
  // 채널 카드(원본 메시지) 업데이트용 정보
  channelMessage?: {
    // UpdateActivity에 필요한 식별자들
    conversationId: string;
    serviceUrl: string;
    activityId: string; // 원본 카드 메시지 id
  };
};
...
function ensureSession(sessionId: string, capacity: number): SessionState {
...
function setChannelMessageRef(sessionId: string, ref: SessionState["channelMessage"]) {
  const store = readStore();
...
function hasBooking(sessionId: string, userId: string): boolean {
  const s = getSession(sessionId);
  return !!s?.bookedUserIds?.includes(userId);
}

function book(sessionId: string, userId: string): { ok: boolean; reason?: "FULL" | "ALREADY_BOOKED"; session: SessionState } {
  const store = readStore();
  const s = store.sessions[sessionId];
  if (!s) throw new Error(`Session not found: ${sessionId}`);

  if (s.bookedUserIds.includes(userId)) return { ok: false, reason: "ALREADY_BOOKED", session: s };
  if (s.bookedUserIds.length >= s.capacity) return { ok: false, reason: "FULL", session: s };
...
function cancel(sessionId: string, userId: string): { ok: boolean; reason?: "ALREADY_CANCELED"; session: SessionState } {
  const store = readStore();
  const s = store.sessions[sessionId];
  if (!s) throw new Error(`Session not found: ${sessionId}`);
  ...
  if (s.bookedUserIds.length === before) return { ok: false, reason: "ALREADY_CANCELED", session: s };
 ...
 */
function buildChannelCard(session: SessionState) {
    ...
    actions: [
      {
        type: "Action.Execute",
        title: "예약하기",
        data: { action: "book", sessionId: session.sessionId, capacity: cap } satisfies CardActionData,
      },
      {
        type: "Action.Execute",
        title: "취소하기",
        data: { action: "cancel", sessionId: session.sessionId, capacity: cap } satisfies CardActionData,
      },
    ],
  };
}

function buildPersonalCard(session: SessionState, viewerHasBooking: boolean) {
  const n = session.bookedUserIds.length;
  const cap = session.capacity;

  const actionData: CardActionData = viewerHasBooking
...
function asAdaptiveCardAttachment(cardJson: any) {
  return {
...
app.message(/^\s*post\s+yoga\s+(\S+)\s+(\d+)\s*$/i, async (context) => {
  ...
  const text = stripMentionsText(context.activity) ?? context.activity.text ?? "";
  ...
  if (conversationId && serviceUrl && activityId) {
    ...
    await context.send(
      "⚠️ 카드 게시는 됐는데, 메시지 업데이트를 위한 참조를 저장하지 못했어. (conversationId/serviceUrl/activityId 누락)"
    );
  }
});
...
app.on("card.action", async (context) => {
  console.log("[bot] card.action received", {
  ...
  const rawData = (value?.action?.data ?? value?.data ?? value) as Partial<CardActionData> | undefined;
  ...
  if (action === "book") {
    ...
    if (viewerHas) {
      ...
      if (!r.ok && r.reason === "FULL") toast = `⛔ 정원이 가득 찼어요. (현재 ${r.session.bookedUserIds.length}/${r.session.capacity})`;
      else toast = "✅ 예약 완료!";
    }
