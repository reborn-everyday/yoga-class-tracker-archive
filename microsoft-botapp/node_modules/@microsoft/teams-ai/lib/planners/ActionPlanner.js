...
class ActionPlanner {
    ...
    async continueTask(context, state, ai) {
        ...
        const augmentation = template.augmentation ?? new augmentations_1.DefaultAugmentation();
     ...
     */
    async completePrompt(context, memory, prompt, validator) {
     ...
     */
    addSemanticFunction(prompt, validator) {
        // Cache prompt template if being dynamically assigned
        let name = '';
        if (typeof prompt == 'object') {
        ...
        this._options.prompts.addFunction(name, async (context, memory, functions, tokenizer, args) => {
            ...
            if (Array.isArray(args)) {
                ...
                memory.setValue('temp.input', args.toString());
            }
            // Complete prompt
            const result = await this.completePrompt(context, memory, prompt, validator);
            if (result.status == 'success') {
                return Utilities_1.Utilities.toString(tokenizer, result.message.content);
            }
            else {
                throw result.error;
            }
