...
exports.AssistantsPlanner = void 0;
...
class AssistantsPlanner {
     ...
     */
    async beginTask(context, state, ai) {
     ...
     * @returns {Promise<Plan>} The plan that was generated.
     */
    async continueTask(context, state, ai) {
     ...
     */
    static async createAssistant(apiKey, request, endpoint, azureClientOptions) {
     ...
     */
    async ensureThreadCreated(state, input) {
     ...
     */
    async waitForRun(thread_id, run_id, handleActions = false) {
        while (true) {
     ...
     */
    async submitActionResults(context, state, ai) {
        const assistantsState = this.ensureAssistantsState(state);
        const actionOutputs = state.temp.actionOutputs;
        const toolMap = state.getValue(SUBMIT_TOOL_OUTPUTS_MAP);
        const toolOutputs = [];
        ...
        const run = await this._client.beta.threads.runs.submitToolOutputsAndPoll(assistantsState.thread_id, assistantsState.run_id, { tool_outputs: toolOutputs });
        ...
        switch (results.status) {
            ...
            case 'expired': {
                const expiredCommand = {
                    type: 'DO',
                    action: AI_1.AI.TooManyStepsActionName,
                    parameters: {}
                };
                return {
                    type: 'plan',
            ...
            default:
                throw new Error(`Run failed ${results.status}. ErrorCode: ${results.last_error?.code}. ErrorMessage: ${results.last_error?.message}`);
        }
    }
    /**
     * @private
     ...
     */
    async submitUserInput(context, state, ai) {
        const threadId = await this.ensureThreadCreated(state, context.activity.text);
     ...
     */
    generatePlanFromTools(state, toolCalls) {
        const plan = { type: 'plan', commands: [] };
        const toolMap = {};
     ...
     */
    async generatePlanFromMessages(thread_id, last_message_id) {
     ...
     */
    updateAssistantsState(state, assistantsState) {
     ...
     * @param {string} thread_id - The ID of the thread.
     */
    async blockOnInProgressRuns(thread_id) {
        while (true) {
     ...
     */
    static createClient(apiKey, endpoint, azureClientOptions) {
        if (endpoint) {
            // Azure OpenAI
            return new openai_1.AzureOpenAI({ endpoint, apiKey, ...azureClientOptions });
        }
        else {
            // OpenAI
...
exports.AssistantsPlanner = AssistantsPlanner;
//# sourceMappingURL=AssistantsPlanner.js.map